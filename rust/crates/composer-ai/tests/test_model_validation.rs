...
fn test_polynomial_model_score_distribution() {
    ...
    let test_cases = vec![
        ...
        (
            "Low complexity", 
            vec![Chord::new(1, 5).unwrap(), Chord::new(5, 5).unwrap(), Chord::new(1, 5).unwrap()],
            100.0,
            (4, 4),
        ...
        (
            "Medium complexity",
            vec![Chord::new(1, 7).unwrap(), Chord::new(6, 7).unwrap(), Chord::new(2, 7).unwrap(), Chord::new(5, 7).unwrap()],
            140.0,
            (4, 4),
        ...
        (
            "High complexity",
            vec![Chord::new(1, 9).unwrap(), Chord::new(6, 11).unwrap(), Chord::new(2, 9).unwrap(), Chord::new(5, 13).unwrap()],
            180.0,
            (7, 8),
        ...
        (
            "Maximum complexity",
            vec![
                Chord::new(1, 13).unwrap(), Chord::new(7, 13).unwrap(), 
                Chord::new(3, 13).unwrap(), Chord::new(2, 13).unwrap(),
                Chord::new(6, 13).unwrap(), Chord::new(4, 13).unwrap()
            ],
            300.0,
            (5, 4),
            8.0..10.0,
    ...
    for (name, progression, tempo, time_sig, expected_range) in test_cases {
        ...
        println!("  {}: Score {:.2} (expected {:.1}-{:.1})", 
            name, score, expected_range.start, expected_range.end);
        
        assert!(expected_range.contains(&score), 
...
fn test_skill_level_classification_boundaries() {
    ...
    let test_cases = vec![
        ("Beginner candidate", beginner_progression, 80.0, SkillLevel::Beginner),
        ("Intermediate candidate", intermediate_progression, 120.0, SkillLevel::Intermediate),
        ("Advanced candidate", advanced_progression, 160.0, SkillLevel::Advanced),
        ("Expert candidate", expert_progression, 200.0, SkillLevel::Expert),
    ];
    ...
    for (name, progression, tempo, expected_level) in test_cases {
        ...
        println!("  {}: Score {:.2}, Level {:?} (expected {:?})", 
...
fn test_weighted_input_calculation() {
    ...
    let manual_weighted_input = components.iter()
        .zip(weights.iter())
        .map(|(comp, weight)| comp * weight)
        .sum::<f64>();
    
    println!("Component weighting test:");
    println!("  Harmonic: {:.2} × {:.0}% = {:.2}", components[0], weights[0] * 100.0, components[0] * weights[0]);
    println!("  Rhythmic: {:.2} × {:.0}% = {:.2}", components[1], weights[1] * 100.0, components[1] * weights[1]);
    println!("  Technical: {:.2} × {:.0}% = {:.2}", components[2], weights[2] * 100.0, components[2] * weights[2]);
    println!("  Melodic: {:.2} × {:.0}% = {:.2}", components[3], weights[3] * 100.0, components[3] * weights[3]);
    ...
    assert!(manual_weighted_input >= 0.0 && manual_weighted_input <= 10.0,
        "Weighted input should be 0-10, got {:.2}", manual_weighted_input);
    
    // Verify harmonic has the highest weight impact
    let harmonic_contribution = components[0] * weights[0];
    let other_contributions = components.iter().skip(1)
...
fn test_model_saturation_prevention() {
    ...
    let progressions = vec![
        ...
        // Moderate
        vec![Chord::new(1, 7).unwrap(), Chord::new(6, 5).unwrap(), Chord::new(4, 5).unwrap(), Chord::new(5, 7).unwrap()],
        ...
        // Very complex
        vec![
            Chord::new(1, 13).unwrap(), Chord::new(8, 11).unwrap(), Chord::new(3, 13).unwrap(), 
            Chord::new(10, 9).unwrap(), Chord::new(6, 13).unwrap(), Chord::new(11, 11).unwrap()
        ],
    ];
    ...
    for (i, progression) in progressions.iter().enumerate() {
        ...
        if let Ok(assessment) = result {
            ...
            println!("  Progression {}: Score {:.2}", i + 1, assessment.overall_score);
        }
    }
    ...
    println!("  Score range: {:.2} - {:.2} (spread: {:.2})", min_score, max_score, score_range);
    
    // We should have at least 3 points of score spread across progressions
    assert!(score_range >= 3.0, 
        "Score range ({:.2}) should be at least 3.0 to avoid saturation", score_range);
    
    // No score should exceed 10.0
    assert!(max_score <= 10.0, 
        "Maximum score ({:.2}) should not exceed 10.0", max_score);
    
    // Minimum score should be reasonable
    assert!(min_score >= 0.0, 
        "Minimum score ({:.2}) should be >= 0.0", min_score);
}
...
fn test_progression_difficulty_ordering() {
    ...
    let progressions = vec![
        ...
        (
            ...
            vec![Chord::new(1, 5).unwrap(), Chord::new(6, 5).unwrap(), Chord::new(4, 5).unwrap(), Chord::new(5, 5).unwrap()],
        ),
        (
            "Seventh chords",
            vec![Chord::new(1, 7).unwrap(), Chord::new(6, 7).unwrap(), Chord::new(2, 7).unwrap(), Chord::new(5, 7).unwrap()],
        ...
        (
            "Complex jazz",
            vec![
                Chord::new(1, 13).unwrap(), Chord::new(3, 11).unwrap(), Chord::new(6, 13).unwrap(), 
                Chord::new(2, 11).unwrap(), Chord::new(5, 13).unwrap(), Chord::new(1, 9).unwrap()
            ],
        ),
    ];
    ...
    for (name, progression) in progressions {
        let assessment = analyzer.assess_difficulty(&progression, Some(120.0), Some((4, 4))).unwrap();
    ...
    for i in 1..results.len() {
        ...
        if curr_score <= prev_score {
            println!("  Warning: {} ({:.2}) not harder than {} ({:.2})", 
