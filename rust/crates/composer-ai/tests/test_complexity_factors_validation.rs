...
fn test_calculate_single_chord_complexity_direct() {
    ...
    assert!(
        ...
        "Triad ({:.2}) should be simpler than seventh ({:.2})",
        triad_complexity,
        seventh_complexity
    );
    assert!(
        seventh_complexity < ninth_complexity,
        "Seventh ({:.2}) should be simpler than ninth ({:.2})",
        seventh_complexity,
        ninth_complexity
    );
    assert!(
        ninth_complexity < eleventh_complexity,
        "Ninth ({:.2}) should be simpler than eleventh ({:.2})",
        ninth_complexity,
        eleventh_complexity
    );
    assert!(
        eleventh_complexity < thirteenth_complexity,
        "Eleventh ({:.2}) should be simpler than thirteenth ({:.2})",
        eleventh_complexity,
        thirteenth_complexity
    );

    // Verify reasonable ranges
    assert!(
        (1.0..=2.0).contains(&triad_complexity),
        "Triad complexity should be 1.0-2.0: {:.2}",
        triad_complexity
    );
    assert!(
        (3.0..=5.0).contains(&thirteenth_complexity),
        "Thirteenth complexity should be 3.0-5.0: {:.2}",
        thirteenth_complexity
    );
}
...
fn test_is_extended_harmony_detection() {
    ...
    println!(
        "  Extended harmonies detected: {}",
        assessment.factors.extended_harmonies
    );
    println!("  Expected extended harmonies: 4 (all except triad)");

    // Should detect 4 extended harmonies (all except the triad)
    assert_eq!(
        assessment.factors.extended_harmonies, 4,
        "Should detect 4 extended harmonies, got {}",
        assessment.factors.extended_harmonies
    );

    // Average chord complexity should be higher than 1.0
    assert!(
        assessment.factors.avg_chord_complexity > 1.5,
...
fn test_average_chord_complexity_calculation() {
    ...
    let manual_simple_avg = simple_progression
        .iter()
        .map(|chord| analyzer.calculate_single_chord_complexity(chord))
        .sum::<f64>()
        / simple_progression.len() as f64;

    let manual_complex_avg = complex_progression
        .iter()
        .map(|chord| analyzer.calculate_single_chord_complexity(chord))
        .sum::<f64>()
        / complex_progression.len() as f64;
    ...
    assert!(
        (simple_assessment.factors.avg_chord_complexity - manual_simple_avg).abs() < 0.01,
    ...
    assert!(
        (complex_assessment.factors.avg_chord_complexity - manual_complex_avg).abs() < 0.01,
    ...
    assert!(
        complex_assessment.factors.avg_chord_complexity
            > simple_assessment.factors.avg_chord_complexity + 1.0,
    ...
    assert_eq!(
        complex_assessment.factors.extended_harmonies, 4,
...
fn test_parallel_iterator_vs_sequential() {
    ...
    let sequential_avg = test_progression
        .iter()
        .map(|chord| analyzer.calculate_single_chord_complexity(chord))
        .sum::<f64>()
        / test_progression.len() as f64;

    // Calculate manually with parallel iterator (like the code does)
    use rayon::prelude::*;
    let parallel_avg = test_progression
        .par_iter()
        .map(|chord| analyzer.calculate_single_chord_complexity(chord))
        .sum::<f64>()
        / test_progression.len() as f64;
    ...
    // Should be identical
    assert!(
        (sequential_avg - parallel_avg).abs() < 0.000001,
        "Sequential ({:.6}) and parallel ({:.6}) should match",
        sequential_avg,
        parallel_avg
    );
    ...
    println!(
        "  Assessment average: {:.6}",
        assessment.factors.avg_chord_complexity
    );

    // Assessment should match manual calculations
    assert!(
        (assessment.factors.avg_chord_complexity - sequential_avg).abs() < 0.01,
...
fn test_complexity_factors_debug_output() {
    ...
    println!(
        "  Unique chords: {}",
        simple_assessment.factors.unique_chords
    );
    println!(
        "  Avg chord complexity: {:.4}",
        simple_assessment.factors.avg_chord_complexity
    );
    println!(
        "  Extended harmonies: {}",
    ...
    println!(
        "  Voice leading complexity: {:.4}",
        simple_assessment.factors.voice_leading_complexity
    );
    println!(
        "  Harmonic complexity: {:.4}",
    ...
    println!(
        "  Unique chords: {}",
        complex_assessment.factors.unique_chords
    );
    println!(
        "  Avg chord complexity: {:.4}",
        complex_assessment.factors.avg_chord_complexity
    ...
    println!(
        "  Voice leading complexity: {:.4}",
        complex_assessment.factors.voice_leading_complexity
    );
    println!(
        "  Harmonic complexity: {:.4}",
...
fn test_caching_issue_diagnosis() {
    ...
    let simple_result = analyzer1
        .assess_difficulty(&simple_progression, Some(120.0), Some((4, 4)))
        .unwrap();
    ...
    let complex_result = analyzer2
        .assess_difficulty(&complex_progression, Some(120.0), Some((4, 4)))
        .unwrap();

    println!("Fresh analyzer test (no shared cache):");
    println!(
        "  Simple - Avg: {:.4}, Extended: {}",
        simple_result.factors.avg_chord_complexity, simple_result.factors.extended_harmonies
    );
    println!(
        "  Complex - Avg: {:.4}, Extended: {}",
        complex_result.factors.avg_chord_complexity, complex_result.factors.extended_harmonies
    );

    // Test 3: Same analyzer, sequential tests (potential cache pollution)
    analyzer.clear_cache(); // Clear any previous cache
    let simple_sequential = analyzer
        .assess_difficulty(&simple_progression, Some(120.0), Some((4, 4)))
        .unwrap();
    let complex_sequential = analyzer
        .assess_difficulty(&complex_progression, Some(120.0), Some((4, 4)))
        .unwrap();
    ...
    println!(
        "  Simple - Avg: {:.4}, Extended: {}",
        simple_sequential.factors.avg_chord_complexity,
        simple_sequential.factors.extended_harmonies
    );
    println!(
        "  Complex - Avg: {:.4}, Extended: {}",
        complex_sequential.factors.avg_chord_complexity,
        complex_sequential.factors.extended_harmonies
    );

    // Test 4: Same analyzer, reverse order
    analyzer.clear_cache();
    let complex_first = analyzer
        .assess_difficulty(&complex_progression, Some(120.0), Some((4, 4)))
        .unwrap();
    let simple_second = analyzer
        .assess_difficulty(&simple_progression, Some(120.0), Some((4, 4)))
        .unwrap();

    println!("\nSame analyzer, reverse order:");
    println!(
        "  Complex first - Avg: {:.4}, Extended: {}",
        complex_first.factors.avg_chord_complexity, complex_first.factors.extended_harmonies
    );
    println!(
        "  Simple second - Avg: {:.4}, Extended: {}",
        simple_second.factors.avg_chord_complexity, simple_second.factors.extended_harmonies
    );

    // All tests should show consistent results
    assert_eq!(
        simple_result.factors.avg_chord_complexity, simple_sequential.factors.avg_chord_complexity,
        "Simple progression should have consistent results across analyzers"
    );
    assert_eq!(
        complex_result.factors.avg_chord_complexity,
        complex_sequential.factors.avg_chord_complexity,
    ...
    assert!(
        complex_result.factors.avg_chord_complexity
            > simple_result.factors.avg_chord_complexity + 1.0,
        "Complex should be significantly harder than simple (fresh analyzers)"
    );
    assert!(
        complex_sequential.factors.avg_chord_complexity
            > simple_sequential.factors.avg_chord_complexity + 1.0,
